...
class Configuration(object):
    ...
    def __init__(self, **kwargs):
        ...
        if self.domains:
            self.domains = [x.strip() for x in str(self.domains).split(',')]
    ...
    def read_from_file(filename):
        ...
        try:
            ...
            raise InvalidConfigFile(
                'Failed to read configuration from "{0}": {1}'.format(filename, e))
...
def configurationWizard():
    ...
    print 'Finished. Please remember to set restrictive permissions if the file contains sensitive data ' + \
          '(like a service password)'
