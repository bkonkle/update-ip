...
class CloudflareService(BaseDNSService):
    ...
    def _update_ip(self, ip, second_try=False):
        ...
        if self.record_id is None:
            ...
            self._get_dns_record()
            self._update_ip(ip, True)
            return

        request = self._create_request(self._update_url,
    ...
    def _get_dns_record(self, second_try=False):
        ...
        if self.zone_id is None:
            ...
            self._get_zones()
            self._get_dns_record(True)
            return

        data = self._api_get_request(self._dns_list_url)
    ...
    def _create_request(self, url, data=None):
        ...
        request = self._add_url_headers(request)
        return request

    def _add_url_headers(self, request):
